...
## D8. `divisions = 0` is a legal mapping

The zero mapping has image `{0}`, which section 1.6 fixes through the
convention `gcd(0, ..., 0) = 0`. Its image has rank 0, so it has no integer
image coordinate at all, and ambient-to-image conversion returns
`TrivialImage` rather than pretending the answer is zero.

## D9. Rank-0 bases are permitted
